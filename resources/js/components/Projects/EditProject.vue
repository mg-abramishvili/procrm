          ...
          <button type="submit" class="bg-blue-500 hover:bg-blue-600 focus:outline-none rounded-lg px-6 py-2 text-white text-sm font-semibold shadow">Сохранить</button>
                    ...
                    <div class="flex flex-col lg:flex-row w-full lg:space-x-2 space-y-2 lg:space-y-0 mb-2 lg:mb-4">
                        <div class="w-full lg:w-1/3">
                            ...
                            <input type="text" class="block w-full text-md rounded-md py-2 px-2 border border-gray-300" v-model="project.title">
                        </div>
                        <div class="w-full lg:w-1/3">
                            <label class="block text-sm font-medium text-gray-700 mb-2">Заказчик</label>
                            <input type="text" class="block w-full text-md rounded-md py-2 px-2 border border-gray-300" v-model="project.client">
                        </div>
                        <div class="w-full lg:w-1/3">
                            <label class="block text-sm font-medium text-gray-700 mb-2">Статус</label>
                            <select v-if="project.status === 'active'" class="block w-full text-md rounded-md py-2 px-2 border border-gray-300" v-model="project.status">
                                <option value="active" selected>в работе</option>
                                <option value="archive">завершен</option>
                            </select>
                            <select v-else class="block w-full text-md rounded-md py-2 px-2 border border-gray-300" v-model="project.status">
                                <option value="active">в работе</option>
                                <option value="archive" selected>завершен</option>
                            </select>
                        </div>
            </div>
            <div class="flex flex-col lg:flex-row w-full lg:space-x-2 space-y-2 lg:space-y-0 mb-2 lg:mb-4">
...
<h2 class="text-xl font-semibold mb-4">Финансы проекта</h2>
                    ...
                    <div class="flex flex-col lg:flex-row w-full lg:space-x-2 space-y-2 lg:space-y-0 mb-2 lg:mb-4">
                        <div class="w-full lg:w-1/3">
                            ...
                            <input type="text" class="block w-full text-md rounded-md py-2 px-2 border border-gray-300" v-model="project.budget">
                        </div>
                    </div>
